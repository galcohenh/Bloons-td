...
public abstract class ImageTool {
	...
	public static Map<Point, BufferedImage> allHeliImages;
    private ImageTool() {
    }
    ...
    public static BufferedImage getEmptyImage(int width, int height){
    	if (allHeliImages == null) {
    		allHeliImages = new HashMap<Point, BufferedImage>();
    	}
    	Point alreadyCreatedPoint = new Point(width, height);
    	BufferedImage alreadyCreatedBuffered = allHeliImages.get(alreadyCreatedPoint);
    	
    	
    	if (alreadyCreatedBuffered != null) {
    		Graphics2D g = alreadyCreatedBuffered.createGraphics();
    		g.setBackground(new Color(0, 0, 0, 0));
    		g.clearRect(0, 0, alreadyCreatedBuffered.getWidth(), alreadyCreatedBuffered.getHeight());
    		return alreadyCreatedBuffered;
    	}
        BufferedImage img = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        allHeliImages.put(alreadyCreatedPoint, img);
        return img;
    }
    ...
    public static boolean isPixelTransparent(BufferedImage img, int x, int y) {
    	try {
